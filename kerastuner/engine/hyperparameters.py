...
class Choice(HyperParameter):
    ...
    def __init__(self, name, values, randomized_values=None, ordered=None, default=None):
    ...
    def __repr__(self):
        return 'Choice(name: "{}", values: {}, randomized_values: {}, ordered: {}, default: {})'.format(
            self.name, self.values, self.randomized_values, self.ordered, self.default)
    ...
    def get_config(self):
        ...
        config['randomized_values'] = self.randomized_values
        return config
    ...
    def from_proto(cls, proto):
        ...
        return cls(
            ...
            randomized_values=proto.randomized_values,
            default=default)
    ...
    def to_proto(self):
        ...
        return kerastuner_pb2.Choice(
            ...
            randomized_values=self.randomized_values,
            values=values,
            default=default)
    ...
    def Choice(self,
        ...
        return self._retrieve(name, 'Choice',
                              config={'values': values,
                                      'randomized_values': randomized_values,
                                      'ordered': ordered,
                                      'default': default},
...
def cumulative_prob_to_value(prob, hp):
    ...
    if isinstance(hp, Fixed):
        ...
        if hp.ordered:
            ...
            return hp.randomized_values[index]
...
def value_to_cumulative_prob(value, hp):
    ...
    if isinstance(hp, Boolean):
        ...
        if not hp.ordered:
            hp.randomized_values = copy.deepcopy(hp.values)
            np.random.shuffle(hp.randomized_values)
        ele_prob = 1 / len(hp.values)
        if not hp.ordered:
            index = hp.randomized_values.index(value)
        else:
